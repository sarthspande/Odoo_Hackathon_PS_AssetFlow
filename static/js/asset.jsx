...
function Icon({ symbol }) {
  return (
    <span className="inline-flex items-center justify-center w-7 h-7 rounded-full bg-slate-200 text-slate-700 text-[12px]">
      {symbol}
    </span>
  );
}
...
function Sidebar() {
  const items = [
    { symbol: "D", label: "Dashboard", active: false },
    { symbol: "A", label: "Assets", active: false },
    { symbol: "T", label: "Allocations", active: true },
    { symbol: "B", label: "Bookings", active: false },
    { symbol: "R", label: "Reports", active: false },
    { symbol: "S", label: "Settings", active: false },
  ];
  ...
  return (
    <div className="w-56 bg-slate-900 text-slate-300 flex flex-col shrink-0">
      <div className="h-16 flex items-center gap-2 px-5 border-b border-slate-800">
        <div className="w-7 h-7 rounded bg-teal-500 flex items-center justify-center text-slate-900">A</div>
        <span className="text-white font-semibold tracking-tight">AssetFlow</span>
      </div>
      <nav className="flex-1 py-4">
        {items.map(({ symbol, label, active }) => (
          <div
            key={label}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="h-16 flex items-center justify-between px-6 border-b border-slate-200 bg-white shrink-0">
          <div>
            ...
            <p className="text-xs text-slate-500">{assets.filter((a) => a.status === "Allocated").length} allocated · {assets.filter((a) => a.status === "Available").length} available</p>
          </div>
          <button
            onClick={() => setAdminMode((v) => !v)}
            className={`flex items-center gap-2 px-3.5 py-2 text-sm font-medium rounded border ${adminMode ? "bg-slate-900 text-white border-slate-900" : "bg-white text-slate-600 border-slate-300"}`}
          >
            Admin mode
                ...
                {pendingTransfers.map((tr) => (
                  <div key={tr.id} className="flex items-center justify-between bg-white border border-amber-200 rounded px-3 py-2">
                    <div className="text-sm text-slate-700">
                      ...
                      {tr.from} → {tr.to}
                      {tr.reason && <span className="text-slate-400 ml-1.5">— "{tr.reason}"</span>}
                    </div>
...
const root = ReactDOM.createRoot(document.getElementById('asset-allocation-root'));
root.render(<AssetAllocation />);
